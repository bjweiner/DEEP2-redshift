...
c 11.29.02 changed the way bounds checking is done when making pstamp,
c          which fixed problem occurring when pstamp was at extreme left
c          added something to check for dw/dpix<0.2 (bad wavecal)
...
c 11.08.02 split the 1-d plot into two windows, lightly and
c          heavily smoothed.  show1dspec now is switchable
c          between median/weighted smoothing
c 10.27.02 made new version zchoose4, which opens
c          two separate windows for 2-d and 1-d
...
c prompt for a quality level and comments?
      parameter(IFQUAL=1)
c show 2 plots, lightly and heavily smoothed, in 1-d window?
      parameter(IFSPLIT1D=1)
...
c      integer function pgopen
c      integer function pgcurs
      integer pgopen
      integer pgcurs
...
c      character answ*1,sline*160
      ...
      character charobjno(maxspectra)*20
...
c open 1-d window device
      idwin1=pgopen('/xw')
      if (idwin1 .le. 0) then
         write(*,*) "Error, couldnt open pgplot device /xw",idwin1
      end if
      call pgsubp(1,1)
      call pgask(.false.)
c open 2-d window device
c      call pgbeg(0,'?',nlplot,nzmax)
      idwin2=pgopen('/xw')
      if (idwin2 .le. 0) then
         write(*,*) "Error, couldnt open pgplot device /xw",idwin2
      end if
...
c try to winkle the object numbers out of the spec1d file name.
c object number is between third and fourth period after 'spec1d'
c for now, object number is from 16 to 23 chars after the 's' in
c 'spec1d'.  this may break later.
c  06/27/03 - indeed it did as e.g. on mask 2101b.  more reliable
c    to find the "fits" and take the -9 to -2 chars from the "f" in "fits"
      do i=1,nobj
c         i1=index(snameoned(i),'spec1d')
c         charobjno(i) = snameoned(i)(i1+16:i1+23)
c         i2=index(snameoned(i)(i1:),'.')
         i1=index(snameoned(i),'fits')
         charobjno(i) = snameoned(i)(i1-9:i1-2)
      end do
...
cc reference both to the good image - this is a kludge.
         if (ifokblue .eq. 0) then
            im1 = im2
         else
            im2 = im1
         end if
...
c entire 2-d array of dwave1 and dwave2
     ...
     $     naxis2,laxes2,specdata)
      call reformat(isize2,specdata,naxis2,laxes2,maxcol,maxrow,dwave2)
...
c make the 1-d plot in its pgplot window
c before interacting with 2-d
      call pgslct(idwin1)
      call pgqch(cheight)
      xpl1=-1.
      xpl2=-1.
      ...
      nsm1d=9
      iplz=1
      if (IFSPLIT1D .eq. 1) then
         call pgsubp(1,2)
         call pgsch(1.8)
         call show1dspec(snameoned(kobj),xpl1,xpl2,ypl1,ypl2,noldsm1d,1)
         call plotalllines(zcand(iplz),nlplot,nlinesets,wlplot,
     $     maxlines,maxlinesets)
         nsm1d=25
         call show1dspec(snameoned(kobj),xpl1,xpl2,ypl1,ypl2,nsm1d,0)
      else
         call pgsch(1.1)
         call show1dspec(snameoned(kobj),xpl1,xpl2,ypl1,ypl2,nsm1d,1)
      end if
      call plotalllines(zcand(iplz),nlplot,nlinesets,wlplot,
     $     maxlines,maxlinesets)
      write(toplabel,'("# ",i2,", z= ",f7.4)') iplz,zcand(iplz)
      call pgmtxt('B',2.5,0.0,0.0,toplabel)
      write(toplabel,'("obj# ",i4,", smoothing = ",i3)') 
     $     kobj,nsm1d
      call pgmtxt('B',2.5,1.0,1.0,toplabel)
      call pgsch(cheight)
c switch back to 2-d
      call pgslct(idwin2)
...
c could determine flux limits for each plot  This never gets used really.
               if (IFSTAT .ne. 0) then
                  npix=0
                  sum=0.
                  sumsq=0.0
                  ...
                  write(*,*) smin,smax,sum/n,sqrt((sumsq-sum*sum/n)/n)
               end if
c end of useless IFSTAT clause
c
...
c check if it's a sky slit
      write(*,'(a,a)') "Object number = ",charobjno(kobj)
      if (charobjno(kobj)(3:3) .eq. '5') then
         ifskyslit=1
         write(*,'(a,a)') "*** From the object number,",
     $        " this is a sky slit ***"
      else
         ifskyslit=0
      end if
...
c switch to interacting with 1-d spectrum
         call pgslct(idwin1)
         call pgqch(cheight)
         call pgsubp(1,1)
c         xpl1=-1.
...
c I decided to replot on 1-d plot
 ...
 440        write(*,'("diameter for smoothing (pixels): ",$)')
            read(*,*,err=440) nsm1d
            nsm1d=max(1,nsm1d)
            go to 430
         else if (cline(1:1) .eq. 'e') then
...
c median or weighted smoothing?
      parameter (IFMEDSMOOTH=1)
      character sname*(*)
      real wave1d(NWAVEMAX),flux1d(NWAVEMAX),ferr1d(NWAVEMAX)
      real tmpspec1(NWAVEMAX),tmpspec2(NWAVEMAX)

      include 'pcredshift.h'

c      ndiamed = 7

c  note I am using tmp1d as a temporary array to store 
c  unsmoothed flux
      call get1ddeepspec(sname,n1d,tmpspec1,ferr1d,w01d,dw1d,ierr)
      if (ierr .ne. 0) then
...
c below the bad-flag
      do i=1,n1d
         if(tmpspec1(i) .gt. 10000.) tmpspec1(i) = badset
      end do
      if (IFMEDSMOOTH .eq. 1) then
         call medsmooth(n1d,tmpspec1,ndiamed,flux1d)
      else
c weightsmooth is broken for some reason
c         call weightsmooth(n1d,tmpspec1,ferr1d,ndiamed,flux1d,tmpspec2)
         call boxcar(n1d,tmpspec1,ndiamed/2,flux1d)
      end if
      do i=1,n1d
...
c      ypl(2)=yw2-0.08*(yw2-yw1)
      ...
      xpl(2)=xpl(1)
      call pgline(2,xpl,ypl) 
      call pgptxt(xpl(1),ypl(2),90.,0.0,' B-band')
      xpl(1) = 7605.
      xpl(2)=xpl(1)
      call pgline(2,xpl,ypl) 
      call pgptxt(xpl(1),ypl(2),90.,0.0,' A-band')
